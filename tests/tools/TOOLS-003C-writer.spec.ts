...
// ---
// Writer C for Multi-Worker Collision Detection
// Feature: tools
// Tag: @tools
// ---

test.describe("TOOLS-003C - Writer C @tools", () => {
  test("TOOLS-003-WRITE C.0", async ({ set }, testInfo) => {
  ...
  test("TOOLS-003-WRITE C.1", async ({ set }, testInfo) => {
    const user = factories.createUser();
    const workerIndex = testInfo.workerIndex;
    const fileLetter = "C";
    const testIndex = 1;
