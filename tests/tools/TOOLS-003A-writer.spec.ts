...
// ---
// Writer A for Multi-Worker Collision Detection
// Feature: tools
// Tag: @tools
// ---

test.describe("TOOLS-003A - Writer A @tools", () => {
  test("TOOLS-003-WRITE A.0", async ({ set }, testInfo) => {
  ...
  test("TOOLS-003-WRITE A.1", async ({ set }, testInfo) => {
    const user = factories.createUser();
    const workerIndex = testInfo.workerIndex;
    const fileLetter = "A";
    const testIndex = 1;
